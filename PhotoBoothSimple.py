...
def main():
	...
	while email1 != email2:
		...
		# validate email
		if email1 == email2 and email1 <> "":
			if validate_email(email2) is True:
				logger.log("main() - Valid email address structure provided.")
				logger.log("*** START OF PICTURE SET FOR %s" % email2)
				...
				uploader.generateShare('/' + scrambledFolderName, email2)
				...
				if ( config.get_EmailEnable() == 1 ):
					emailer.sendEmail(email2, flatList)

			else:
				logger.log_warning('Email Address %s has an invalid format.' % email1)
				continue
		else:
			logger.log_warning('The email addresses are different. Email1: %s <> Email2 %s' % (email1, email2) )
			continue
